...
export class GenericCrudListComponent implements OnInit, OnDestroy {
  ...
  private getDependencyItemLabel(item: any): string {
    ...
    if (item.id_annotation) {
      const desc = item.description
        ? ` — "${String(item.description).slice(0, 60)}${item.description.length > 60 ? '…' : ''}"`
        : '';
      return `Anotación #${item.id_annotation}${desc}`;
    }
    if (item.id_interested_party) {
      const annotation = item.id_annotation ? ` (anotación #${item.id_annotation})` : '';
      return `Interesado #${item.id_interested_party}${annotation}`;
    }
    if (item.id_official) return `Funcionario #${item.id_official}`;
    if (item.id_citizen) return `Ciudadano #${item.id_citizen}`;
